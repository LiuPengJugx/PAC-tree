...
def transform_query_to_sql(
    hdfs_path, query, num_dims, column_names, querytype=0, pids=None
):
    sql = ""
    where_clause = ""
    filter_columns = []
    ...
    for dim in range(num_dims):
        ...
        if query[dim][0] > domains[dim][0] or query[dim][1] < domains[dim][1]:
            filter_columns.append(column_names[dim])
    ...
    if querytype == 0:
        ...
        sql = (
            "SELECT variance(_c0) FROM parquet.`"
            + pq_hdfs_path
            + "`WHERE "
            + where_clause
        )
        ...
        for pid in pids:
            sql.append(
                f"SELECT * FROM parquet.`{hdfs_path + '/partition_' + str(pid) + '.parquet'}`"
            )
...
def query_with_parquets(
    ...
    if partition_tree == None:
        sql, _, _ = transform_query_to_sql(
            hdfs_path, query[1], num_dims, column_names, querytype
        )
    else:
        #       print("pids:", pids)
        if len(pids) > 0:
            sql, where_clause, filter_columns = transform_query_to_sql(
                hdfs_path, query[1], num_dims, column_names, querytype, pids
            )
        ...
        for _ in range(repeated_times):
            ...
            if print_lower_bound:
                ...
                if len(query_result) <= 0:
                    ...
                    b_size = block_size * 2
                    lb_hdfs_path = (
                        f"{hdfs_base_path}/{suffix}/{benchmark}/{tab}/LB-Cost/merged"
                    )
                    ...
                    def update_temp_files(mode="mkdir"):
                        for b_counter in range(b_num):
                            lb_partition_path = (
                                lb_hdfs_path + f"/partition_lb_{b_counter}.parquet"
                            )
                            if mode == "mkdir":
                    ...
                    if querytype == 5:
                        ...
                        for b_counter in range(b_num):
                            ...
                            spark.sql(lb_sql).select(filter_columns).where(
                                where_clause
                            ).collect()
    ...
    return (
        query_translation_time,
        query_execution_time / repeated_times,
        actual_data_size1,
        data_size_list,
        vaild,
    )
...
def batch_query(queryset, benchmark, tab, partition_tree, hdfs_path, querytype=0):
    ...
    for i in range(0, len(queryset)):
        result = query_with_parquets(
            hdfs_path, queryset[i], num_dims, col_names, querytype, partition_tree
        )
        if result[4]:
    ...
    for result in results:
        ...
        accumulate_response_time.append(
            sum(mean_response_time) / len(mean_response_time)
        )
    ...
    return_resp = [
        ...
        latency_df.iloc[1],
        latency_df.iloc[2],
        min(mean_response_time),
        sum(mean_response_time) / len(mean_response_time),
    ]
    print(
        f"Query response time: 75th->{return_resp[0]:5.3f} 90th->{return_resp[1]:5.3f} 95th->{return_resp[2]:5.3f} min->{return_resp[3]:5.3f}, mean->{return_resp[4]:5.3f}"
    )
    return return_resp
...
def getQueryTuple(benchmark, tab, partition_tree, suffix):
    ...
    for qid, q in enumerate(queries):
        print(qid)
        query_boundary = partition_tree.pt_root.approximate_bound_for_query(
            q, encode_dicts, True
        )
...
def getDynamicQueryTuple(benchmark, tab, partition_tree):
    ...
    _, domains, domain_ids = encode_real_queries(
        suffix="similar-queries", tab=tab, benchmark=benchmark
    )
    with open(f"{query_common_path}/{benchmark}_{tab}_test.pickle", "rb") as f:
        train_test_set = pickle.load(f)
        train_set, test_set = np.hsplit(
            recover_query_from_norm(train_test_set, len(domains), domains), 2
        )
...
for strategy in [5]:  # [2,5,6]
    if strategy == 6:  # lb-COST
        ...
        partition_tree.load_tree(
            f"{layout_base_path}/{suffix}/{benchmark}_{tab}_{strategy_tree_dict[2]}"
        )
        hdfs_path = f"{hdfs_base_path}/{suffix}/{benchmark}/{tab}/{strategy_tree_dict[2]}/merged"
    else:
        partition_tree.load_tree(
            f"{layout_base_path}/{suffix}/{benchmark}_{tab}_{strategy_tree_dict[strategy]}"
        )
    ...
    if queryset is None:
        ...
        # print(queryset)
    latency = batch_query(
        queryset, benchmark, tab, partition_tree, hdfs_path, querytype=querytype
    )
    exp_result_dict[strategy] = latency
if suffix == "numeric":
    pd.DataFrame(
        data=exp_result_dict.values(), index=list(exp_result_dict.keys())
    ).to_csv(
        "/home/liupengju/pycharmProjects/dasfa/spark/spark_latency/"
        + benchmark
        + "_"
        + tab
        + "_numeric_res.csv",
        header=["75th", "90th", "95th", "min", "mean"],
    )
else:
    pd.DataFrame(
        data=exp_result_dict.values(), index=list(exp_result_dict.keys())
